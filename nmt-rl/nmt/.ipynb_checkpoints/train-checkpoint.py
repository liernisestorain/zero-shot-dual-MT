...
from . import attention_model_RL
from . import gnmt_model_RL
from . import inference
from . import model as nmt_model
from . import model_RL as nmt_model_RL
...
def run_external_eval(infer_model, infer_sess, model_dir, hparams,
  ...
  '''
  dev_names = ["3devset-en2fr2", "3devset-fr2en2", "3devset-en2es2", "3devset-es2en2", "3devset-fr2es2", "3devset-es2fr2"]
  test_names = ["3testset-en2fr2", "3testset-fr2en2", "3testset-en2es2", "3testset-es2en2", "3testset-fr2es2", "3testset-es2fr2"]


  for f_dev, f_test in zip(dev_names, test_names):
    print("*************************", f_dev)
    print("*************************", f_test)
    
    dev_src_file = "%s.%s" % ("/home/lierni309/final/data/" + f_dev, hparams.src)
    dev_tgt_file = "%s.%s" % ("/home/lierni309/final/data/" + f_dev, hparams.tgt)
    dev_infer_iterator_feed_dict = {
        infer_model.src_placeholder: inference.load_data(dev_src_file),
        infer_model.batch_size_placeholder: hparams.infer_batch_size,
    }
    ...
    test_scores = None
    if use_test_set and hparams.test_prefix:
      test_src_file = "%s.%s" % ("/home/lierni309/final/data/" + f_test, hparams.src)
      test_tgt_file = "%s.%s" % ("/home/lierni309/final/data/" + f_test, hparams.tgt)
      test_infer_iterator_feed_dict = {
          infer_model.src_placeholder: inference.load_data(test_src_file),
          infer_model.batch_size_placeholder: hparams.infer_batch_size,
      }
    ...
    print("/////////////////////////////////// {} BLEU: {}".format(f_dev, dev_scores))
    print("////////////////////////////////// {} BLEU: {}".format(f_test, test_scores))
  '''
...
def train(hparams, scope=None, target_session=""):
  ...
  steps_per_eval = 2 * steps_per_stats  # TODO 10 * steps_per_stats
  if not steps_per_external_eval:
    steps_per_external_eval = steps_per_eval # TODO 5 * steps_per_eval
  ...
  elif hparams.attention_architecture == "gnmt_RL":
    model_creator = gnmt_model_RL.GNMTModel
  else:
  ...
  if latest_ckpt:
    print("Retraining. Restoring model...")
  else :
    print("Initializing weights with baseline values...")
    ...
    for metric in hparams.metrics:
      ...
      if save_on_best and scores[metric] > getattr(hparams, "best_" + metric):
        ...
        model.saver.save(
          ...
          write_meta_graph=False)
      # TODO check this write_meta_graph=False... Check it everywhere
