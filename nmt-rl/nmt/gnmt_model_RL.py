...
class GNMTModel(attention_model_RL.AttentionModel):
  ...
  def _build_encoder(self, hparams):
    ...
    if self.backtranslation:
        print("Backtranslation encoder working here with source sampling ids")
        source = self.sampling_ids
        with tf.variable_scope("encoder") as scope:
            ...
            bi_encoder_outputs, bi_encoder_state = self._build_bidirectional_rnn(
                ...
                sequence_length=self.sampling_lengths,
                dtype=dtype,
                hparams=hparams,
    ...
    print("Original encoder working here with source from iterator")

    #source = tf.Print(source, [self.batch_size, source], summarize=300)
    ...
    return encoder_outputs, encoder_state, source
  ...
  def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state,
    ...
    elif attention_architecture in ["gnmt_v2", "gnmt_RL"]:
      cell = GNMTAttentionMultiCell(
